#!/usr/bin/env bash
# Export the vanilla disco ball party loop (Sound_LevelObject_Tram_Discoball_Partymusic_Loop) from MIMESIS Unity assets.
#
# Output: src/MimesisPlayerEnhancement/Assets/DiscoBallSound/vanilla.wav
#
# Usage:
#   ./scripts/export-disco-ball-sound.sh
#   ./scripts/export-disco-ball-sound.sh --game /path/to/MIMESIS
#   MIMESIS_PATH=/path/to/MIMESIS ./scripts/export-disco-ball-sound.sh
#
...
# export-round-start-sound.sh.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUTPUT="$ROOT/src/MimesisPlayerEnhancement/Assets/DiscoBallSound/vanilla.wav"
GAME_PATH=""
PYTHON_ARGS=(
  --name Sound_LevelObject_Tram_Discoball_Partymusic_Loop
)

usage() {
  cat <<EOF
Export disco ball party loop (Sound_LevelObject_Tram_Discoball_Partymusic_Loop) into mod embedded assets.

Usage: $(basename "$0") [options]

Options:
  --game <path>       MIMESIS install root (else MIMESIS_PATH / PathConfig.props)
  --output <path>     Output file (default: Assets/DiscoBallSound/vanilla.wav)
...
run_docker() {
  ...
  docker run --rm \
    ...
    -v "$ROOT:/repo" \
    python:3.12-slim \
    bash -lc "pip install -q -t /tmp/unitypy UnityPy && PYTHONPATH=/tmp/unitypy python /repo/scripts/export-round-start-sound.py --game /game --output '$container_out' --name Sound_LevelObject_Tram_Discoball_Partymusic_Loop"
}
