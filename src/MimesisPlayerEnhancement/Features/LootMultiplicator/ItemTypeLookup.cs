...
internal static class ItemTypeLookup
{
    ...
    {
        if (!TryGetItem(masterId, out ItemMasterInfo info))
            return ItemType.Miscellany;

        return NormalizeItemType(info.ItemType);
    }
    ...
    {
        if (info == null && !TryGetItem(masterId, out info))
            return masterId.ToString();

        return string.IsNullOrWhiteSpace(info!.Name) ? masterId.ToString() : info.Name;
    }

    internal static ItemType NormalizeItemType(ItemType itemType) =>
        itemType.Equals(ItemType.Consumable) ? ItemType.Consumable
