...
With `EnableLoadingWaitPlayerList`, a spaced player list appears in a bottom roster strip on `wait.png` during this wait phase (~**70px** tall at 1080p, inset **10px** from the bottom/left/right): white names for loaded players, red for those still loading, and green while a player is talking. Names wrap into at most **2** rows centered in that strip (on 21:9 and wider displays, the band tracks the pillarboxed image rather than the full screen width). Paint the darker strip at the bottom of `wait.png` for this roster. Disable wait-phase pan/zoom (`phases.wait.motion.mode: "none"`) when that band is authored, or the image may drift under the list. The list fades in with the wait-image crossfade and fades out with the custom loading overlay when the scene finishes loading.
