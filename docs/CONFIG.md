...
| `CustomLoadingScreenMode` | string | `Random` | `Vanilla`, `Random`, `Specific` | Replace scene loading overlay art with embedded PNG themes. Dungeon transitions can use separate `loading.png` and `wait.png` from the same theme folder. Client-only. |
