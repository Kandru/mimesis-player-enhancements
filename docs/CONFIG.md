...
| `DiscoBallSoundMode` | string | `Vanilla` | `Vanilla`, `Random`, `Specific` | Replace the loop that plays when the tram disco ball is turned on. The mirror-ball button click stays vanilla. `Random`/`Specific` fall back to vanilla when no tracks are embedded. Your game only. |
...
| `DiscoBallSoundRandomPool` | string | *(empty)* | comma-separated variant ids | Random mode only. When empty, any embedded track may be picked. When set, only listed variant ids are eligible. The pick is held for the whole dungeon and survives toggling the ball off and on. Your game only. |
| `DiscoBallSoundVolume` | float | `0.8` | `0`–`1` | Playback volume for custom disco ball music (`Random` or `Specific` mode). Does not affect vanilla mode. Your game only. |
